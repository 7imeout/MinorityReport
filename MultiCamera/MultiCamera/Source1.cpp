...
state *initializePointerLib() try
{
...
{
	std::cerr << "RealSense error calling " << e.what() << std::endl;
	return 0;
}

bool pointerNextFrame(int &xInOut, int &yInOut, int &zInOut) try
{
	...
	if (handPoint != cv::Point(0, 0))
		cv::circle(depth8u, handPoint, 10, 128, -1);
...
int main1(int argc, char * argv[]) try
{
	rs::log_to_console(rs::log_severity::warn);

	state *app_state = initializePointerLib();
	if (app_state == 0)
	{
	...
	while (!glfwWindowShouldClose(win))
	{
		...
		cv::Point handPoint(0, 0);
		int z = 0;
		bool handPresent = pointerNextFrame(handPoint.x, handPoint.y, z);
